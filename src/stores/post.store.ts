...
export const usePostStore = defineStore('post_store', {
  ...
  actions: {
    async fetchPostMenuByFolderPath(folderPath: string, config?: { cache?: RequestCache }) {
      ...
      //   const urlPath = ESString.joinPath('..', FOLDER_ROOT, folderPath, POSTS_MENU_NAME)
      //   const response = await fetch(urlPath)
      //   const responseJson = await response.json()
      //   return {
      //     postMenuRawList: responseJson as PostMenuRaw[],
      //     postMenuSha: new Date().toISOString(),
      //   }
      // }
      ...
      return {
        postMenuRawList: JSON.parse(fetchResponse.content) as PostMenuRaw[],
        postMenuSha: fetchResponse.sha,
      }
    },

    async fetchPostContentByFilePath(filePath: string, config?: { cache?: RequestCache }) {
      // if (CONFIG.MODE === 'development') {
      //   const urlPath = ESString.joinPath('..', FOLDER_ROOT, filePath)
      //   const response = await fetch(urlPath)
    ...
    async getPostContentByFilePath(filePath: string, config?: { refetch?: boolean; cache?: RequestCache }) {
      ...
      if (config?.refetch || !this.folderDataMap[parentPath]?.postContentMap?.[fileName]) {
        const postContentFetchRes = await this.fetchPostContentByFilePath(filePath, {
          cache: config?.cache,
        })
