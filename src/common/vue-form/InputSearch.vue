...
import { IconSearch } from '../icon/icon-antd'
...
const updateDropdownPosition = async () => {
  ...
  const dropdownHeight = Math.min(dropdownRef.value?.offsetHeight || props.maxHeight, props.maxHeight)
  ...
  dropdownStyle.value = {
    ...
    top: showAbove ? `${rect.top + window.scrollY - dropdownHeight}px` : `${rect.bottom + window.scrollY}px`,
  }
}
...
const setScrollOption = (type: 'ArrowUp' | 'ArrowDown') => {
  nextTick(() => {
    ...
    if (topItem > dropdownRef.value.scrollTop && bottomItem < dropdownRef.value.scrollTop + props.maxHeight) {
      return
    }
...
<template>
  <div v-click-outside="handleClickOutside" :class="{ 'vue-input': true, disabled }" ref="wrapperRef">
