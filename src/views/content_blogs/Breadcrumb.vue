...
import { IconHome } from '../../common/icon/icon-antd'
...
watch(
  () => route.path,
  (path) => {
    const segmentList = path.replace('/blogs/category', '').replace('/blogs/post', '').split('/')
  ...
  <nav class="breadcrumb" aria-label="Breadcrumb">
    <ol>
      ...
      <li v-for="(item, index) in breadcrumbItems" :key="`${item.label}-${index}`">
        <RouterLink :to="`/blogs/category/${item.to}`">{{ item.label }}</RouterLink>
      </li>
    </ol>
  </nav>
</template>
